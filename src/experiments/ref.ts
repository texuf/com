...
const logoAnimations: {
  openai: LogoAnimationState;
  claude: LogoAnimationState;
} = {
...
function getPreparedSingleLineWidth(
...
function headlineBreaksInsideWord(
  prepared: PreparedTextWithSegments,
  maxWidth: number,
): boolean {
...
function projectedBodyLinesEqual(
  a: ProjectedBodyLine[],
  b: ProjectedBodyLine[],
): boolean {
  if (a.length !== b.length) return false;
...
function updateLogoSpin(logo: LogoAnimationState, now: number): boolean {
  ...
  logo.angle =
    logo.spin.from + (logo.spin.to - logo.spin.from) * easeSpin(progress);
  if (progress >= 1) {
...
function buildLayout(
  ...
  if (isNarrow) {
    ...
    const columnWidth = Math.round(
      Math.min(pageWidth - gutter * 2, NARROW_COLUMN_MAX_WIDTH),
    );
    const headlineTop = 28;
    const headlineWidth = pageWidth - gutter * 2;
    const headlineFontSize = Math.min(
      48,
      fitHeadlineFontSize(headlineWidth, pageWidth),
    );
    ...
    const claudeSize = Math.round(
      Math.min(92, pageWidth * 0.23, pageHeight * 0.11),
    );
  ...
  const headlineTop = Math.round(
    Math.max(42, pageWidth * 0.04, HINT_PILL_SAFE_TOP),
  );
  ...
  const claudeSize = Math.round(
    Math.max(276, Math.min(500, pageWidth * 0.355, pageHeight * 0.45)),
  );
  const headlineRegion: Rect = {
    x: gutter,
    y: headlineTop,
...
function evaluateLayout(
  ...
  const headlineBottom =
    headlineLines.length === 0
      ...
      : Math.max(
          ...headlineLines.map((line) => line.y + layout.headlineLineHeight),
        );
